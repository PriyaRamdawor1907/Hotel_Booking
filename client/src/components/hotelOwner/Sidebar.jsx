...
const Sidebar = () =>{
  ...
  return(
    <div className="md:w-64 w-16 border-r h-full text-base border-b-gray-300 pt-4
    flex flex-col transition-all duration-300">
      {sidebarLinks.map((item,index) =>(
        <NavLink to={item.path} key={index} end='/owner' 
        className={({isActive}) => `flex items-center py-3 px-4 md:px-8 gap-3
        ${isActive ? "border-r-4 md:border-r-[6px] bg-teal-900/10 border-teal-700 text-teal-900":
